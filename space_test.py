...
LANE_POSITIONS = [200, 300, 400, 500]
ROAD_LEFT_EDGE = 150
ROAD_RIGHT_EDGE = WIDTH - 150
...
class Player:
    ...
    def move(self, keys):
        if (keys[pygame.K_LEFT] or keys[pygame.K_a]) and self.x > ROAD_LEFT_EDGE:
            self.x -= self.speed
        if (keys[pygame.K_RIGHT] or keys[pygame.K_d]) and self.x < ROAD_RIGHT_EDGE - self.width:
            self.x += self.speed
...
class ObstacleCar:
    def __init__(self):
        ...
        self.color = random.choice([RED, GREEN, YELLOW, ORANGE])
        self.shape = 'circle'
...
class Road:
    ...
    def draw(self, surface):
        # Road background
        pygame.draw.rect(surface, GRAY, (ROAD_LEFT_EDGE, 0, ROAD_RIGHT_EDGE - ROAD_LEFT_EDGE, HEIGHT))
        # Road edges
        pygame.draw.rect(surface, WHITE, (ROAD_LEFT_EDGE, 0, 10, HEIGHT))
        pygame.draw.rect(surface, WHITE, (ROAD_RIGHT_EDGE - 10, 0, 10, HEIGHT))
...
def main():
    ...
    while running:
        ...
        while game_active:
            ...
            # Draw UI
            draw_text(screen, "Space Shooter - Dodge Enemies!", 36, 80, 10)
