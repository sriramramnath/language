...
class CodeGenerator:
    ...
    def emit_game_initialization(self):
        ...
        if self.game_config:
            ...
            self.emit("icon.fill((0, 100, 255))  # Blue background")
            self.emit("pygame.draw.rect(icon, (255, 255, 255), (8, 8, 16, 16))  # White square")
            self.emit("pygame.display.set_icon(icon)")
            self.dedent()
            self.emit("except:")
            self.indent()
            self.emit("pass  # If icon setting fails, continue without it")
            self.dedent()
        else:
            self.emit("# Set up display")
